...
public class EffectSprite extends Image {
    ...
    private static HashMap<Char, EffectSprite> all = new HashMap<Char, EffectSprite>();
    ...
    public static void show(Char ch, int index) {
        ...
        EffectSprite old = all.get(ch);
        if (old != null) {
            old.kill();
        }

        EffectSprite sprite = GameScene.spellSprite();
        sprite.revive();
        sprite.reset(index);
        sprite.target = ch;
        all.put(ch, sprite);
