...
import com.wafitz.pixelspacebase.sprites.MaskedPrisonerSprite;
...
public class MaskedPrisoner extends Mob {

    {
        spriteClass = MaskedPrisonerSprite.class;

        //HP = HT = 120; - one shot for testing
        HP = HT = 1;
        EXP = 20;
        defenseSkill = 20;
    ...
    public void damage(int dmg, Object src) {
        ...
        if (beforeHitHP > HT / 2 && HP <= HT / 2) {
            ...
            //if masked prisoner has lost a certain amount of hp, jump
    ...
    public boolean isAlive() {
        return SpacebaseRun.level.mobs.contains(this); //MaskedPrisoner has special death rules, see prisonbosslevel.progress()
    }

    @Override
    public void die(Object cause) {

        if (SpacebaseRun.hero.subClass == HeroSubClass.NONE) {
            SpacebaseRun.level.drop(new TrainingManual(), pos).sprite.drop();
        }
    ...
    //masked prisoner's attack is always visible
    @Override
    ...
    private void jump() {
        ...
        for (int i = 0; i < 4; i++) {
            int ventPos;
            do {
                ventPos = Random.Int(SpacebaseRun.level.length());
        ...
        if (HP > HT / 2) {
            do {
                newPos = Random.Int(SpacebaseRun.level.length());
            } while (
                    !(SpacebaseRun.level.map[newPos] == Terrain.INACTIVE_VENT || SpacebaseRun.level.map[newPos] == Terrain.VENT) ||
                            Level.solid[newPos] ||
                            SpacebaseRun.level.adjacent(newPos, enemy.pos) ||
            ...
            do {
                newPos = Random.Int(SpacebaseRun.level.length());
                    ...
                    Level.solid[newPos] ||
                            SpacebaseRun.level.distance(newPos, enemy.pos) < 8 ||
                            Actor.findChar(newPos) != null);
        }

        if (SpacebaseRun.visible[pos]) CellEmitter.get(pos).burst(Speck.factory(Speck.WOOL), 6);


        sprite.move(pos, newPos);
        move(newPos);

        if (SpacebaseRun.visible[newPos]) CellEmitter.get(newPos).burst(Speck.factory(Speck.WOOL), 6);
    ...
    public void notice() {
        ...
        if (HP == HT) {
            yell(Messages.get(this, "notice_mine", SpacebaseRun.hero.givenName()));
        } else {
            yell(Messages.get(this, "notice_face", SpacebaseRun.hero.givenName()));
        }
    }
    ...
    //masked prisoner is always hunting
