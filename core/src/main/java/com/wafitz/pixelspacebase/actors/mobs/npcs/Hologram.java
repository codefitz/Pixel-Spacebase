...
import com.wafitz.pixelspacebase.windows.WndHologram;
    ...
    public boolean interact() {
        ...
        if (Quest.given) {
            if (Quest.weapon != null) {
                if (Quest.processed) {
                    GameScene.show(new WndHologram(this, Quest.type));
                } else {
                    switch (Quest.type) {
                        case 1:
                        default:
    ...
    public static class Quest {
        ...
        public static void spawn(OperationsLevel level) {
            if (!spawned && Dungeon.depth > 1 && Random.Int(5 - Dungeon.depth) == 0) {
                ...
                do {
                    hologram.pos = level.randomRespawnCell();
                } while (hologram.pos == -1);
