...
public class ScriptPane extends Component {
    ...
    public ScriptPane(Component content) {
        super();
    ...
    public void scriptTo(float x, float y) {
        content.camera.script.set(x, y);
    }
    ...
    private class TouchController extends TouchArea {
        ...
        protected void onTouchUp(Touch touch) {
            if (dragging) {
                ...
                ScriptPane.this.onClick(p.x, p.y);
        ...
        protected void onDrag(Touch t) {
            if (dragging) {
                ...
                c.script.offset(PointF.diff(lastPos, t.current).invScale(c.zoom));
                if (c.script.x + width > content.width()) {
                    c.script.x = content.width() - width;
                }
                if (c.script.x < 0) {
                    c.script.x = 0;
                }
                if (c.script.y + height > content.height()) {
                    c.script.y = content.height() - height;
                }
                if (c.script.y < 0) {
                    c.script.y = 0;
                }

                thumb.y = y + height * c.script.y / content.height();
