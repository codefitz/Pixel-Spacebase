...
public class GravityGun extends EquippableModule {
    ...
    private CellSelector.Listener caster = new CellSelector.Listener() {
        ...
        public void onSelect(Integer target) {
            if (target != null && (SpacebaseRun.level.visited[target] || SpacebaseRun.level.mapped[target])) {

                //ballistica does not go through walls on pre-rework boss arenas
                int missileProperties = (SpacebaseRun.depth == 10 || SpacebaseRun.depth == 15 || SpacebaseRun.depth == 20 || SpacebaseRun.depth == 25) ?
                ...
                if (Actor.findChar(pull.collisionPos) != null) {
                    ...
                    if (newPos == -1) {
                        ...
                        int chargeUse = SpacebaseRun.level.distance(affected.pos, newMobPos);
                        if (chargeUse > charge) {
                        ...
                        curUser.sprite.parent.add(new GravityTether(curUser.pos, affected.pos, new Callback() {
                            public void call() {
                                Actor.add(new Pushing(affected, affected.pos, newMobPos, new Callback() {
                                    public void call() {
                                        SpacebaseRun.level.press(newMobPos, affected);
                                    }
                                }));
                    ...
                    if (newPos == -1) {
                        ...
                        int chargeUse = SpacebaseRun.level.distance(curUser.pos, newHeroPos);
                        if (chargeUse > charge) {
                            GLog.w(Messages.get(GravityGun.class, "no_charge"));
                            return;
                        } else {
                        ...
                        curUser.sprite.parent.add(new GravityTether(curUser.pos, target, new Callback() {
                            public void call() {
                                Actor.add(new Pushing(curUser, curUser.pos, newHeroPos, new Callback() {
                                    public void call() {
                                        SpacebaseRun.level.press(newHeroPos, curUser);
                                    }
                                }));
                                curUser.spendAndNext(1f);
    ...
    public class gravityRecharge extends ModuleBuff {
