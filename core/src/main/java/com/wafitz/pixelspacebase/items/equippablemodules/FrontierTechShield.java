...
public class FrontierTechShield extends EquippableModule {
    ...
    public class Naturalism extends ModuleBuff {
        public void charge() {
        ...
        public void onSelect(Item item) {
            if (item != null && item instanceof Mine.Device) {
                if (devices.contains(item.getClass())) {
                    GLog.w(Messages.get(FrontierTechShield.class, "already_applied"));
                } else {
                    ...
                    if (devices.size() >= 3 + (level() * 3)) {
                        ...
                        upgrade();
                        if (level() >= 1 && level() <= 3) {
                            GLog.p(Messages.get(FrontierTechShield.class, "levelup"));
                        }

                    } else {
                        GLog.i(Messages.get(FrontierTechShield.class, "absorb_device"));
                    }
