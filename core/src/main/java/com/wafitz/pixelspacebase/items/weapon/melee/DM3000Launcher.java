...
public class DM3000Launcher extends MeleeWeapon {
    ...
    {
        image = ItemSpriteSheet.DM3000_ROCKET_LAUNCHER;
    ...
    public DM3000Launcher(Blaster blaster) {
        this();
        blaster.identify();
        ...
        name = Messages.get(blaster, "launcher_name");
    }
    ...
    public void activate(Char ch) {
        if (blaster != null) blaster.charge(ch, LAUNCHER_SCALE_FACTOR);
    }
    ...
    public int reachFactor(Hero hero) {
        ...
        if (blaster instanceof Disintegrator && hero.subClass == HeroSubClass.BATTLEMAGE) {
            reach++;
        }
        return reach;
    }
    ...
    public boolean collect(Container container) {
        if (super.collect(container)) {
            if (container.owner != null && blaster != null) {
                blaster.charge(container.owner, LAUNCHER_SCALE_FACTOR);
            }
            return true;
        } else {
            return false;
    ...
    public Item convertBlaster(Blaster blaster, Char owner) {
        ...
        int launcherLevelDiff = targetLevel - this.level();
        if (launcherLevelDiff > 0)
            this.upgrade(launcherLevelDiff);
        else if (launcherLevelDiff < 0)
            this.degrade(Math.abs(launcherLevelDiff));
    ...
    public Emitter emitter() {
        ...
        emitter.pour(LauncherParticleFactory, 0.1f);
        return emitter;
    }
    ...
    private final WndContainer.Listener itemSelector = new WndContainer.Listener() {
        ...
        public void onSelect(final Item item) {
            if (item != null) {
                ...
                if (blaster == null) {
                    ...
                    GameScene.show(
                            new WndOptions("",
                                    Messages.get(DM3000Launcher.class, "warning", newLevel),
                                    Messages.get(DM3000Launcher.class, "yes"),
                                    Messages.get(DM3000Launcher.class, "no")) {
                                @Override
        ...
        private void applyBlaster(Blaster blaster) {
            ...
            GLog.p(Messages.get(DM3000Launcher.class, "load", blaster.name()));
    ...
    private final Emitter.Factory LauncherParticleFactory = new Emitter.Factory() {
        @Override
        //reimplementing this is needed as instance creation of new staff particles must be within this class.
        public void emit(Emitter emitter, int index, float x, float y) {
            launcherParticle c = (launcherParticle) emitter.getFirstAvailable(launcherParticle.class);
            if (c == null) {
                c = new launcherParticle();
                emitter.add(c);
            }
            c.reset(x, y);
        }
        ...
        public boolean lightMode() {
            return !((blaster instanceof Disintegrator)
                    || (blaster instanceof DominationBlaster)
                    || (blaster instanceof EMP));
        }
    };

    //determines particle effects to use based on blaster the staff owns.
    public class launcherParticle extends PixelParticle {

        private float minSize;
        private float maxSize;
        public float sizeJitter = 0;

        public launcherParticle() {
            super();
        }
