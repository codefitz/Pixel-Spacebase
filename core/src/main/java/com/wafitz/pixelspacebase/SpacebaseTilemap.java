...
public class SpacebaseTilemap extends Tilemap {
    ...
    public int screenToTile(int x, int y) {
        Point p = camera().screenToCamera(x, y).
                ...
                floor();
        return p.x >= 0
                && p.x < SpacebaseRun.level.width()
                && p.y >= 0
                && p.y < SpacebaseRun.level.height() ?
                p.x + p.y * SpacebaseRun.level.width()
                : -1;
    }
    ...
    public static PointF tileToWorld(int pos) {
        return new PointF(pos % SpacebaseRun.level.width(), pos / SpacebaseRun.level.width()).scale(SIZE);
    }

    public static PointF tileCenterToWorld(int pos) {
        return new PointF(
                (pos % SpacebaseRun.level.width() + 0.5f) * SIZE,
                (pos / SpacebaseRun.level.width() + 0.5f) * SIZE);
    }
