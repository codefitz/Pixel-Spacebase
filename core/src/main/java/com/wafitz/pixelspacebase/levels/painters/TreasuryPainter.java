...
public class TreasuryPainter extends Painter {
    ...
    public static void paint(Level level, Room room) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            level.drop(new Parts().random(), pos).type = (Random.Int(20) == 0 && heapType == Heap.Type.CHEST ? Heap.Type.CONFUSEDSHAPESHIFTER : heapType);
        }
        ...
        if (heapType == Heap.Type.HEAP) {
            for (int i = 0; i < 6; i++) {
                ...
                level.drop(new Parts(Random.IntRange(5, 12)), pos);
            }
        }
