...
public class WndTextInput extends Window {
    ...
    public WndTextInput(final String title, final String initialValue, final int maxLength,
        ...
        final int width;
        if (PixelSpacebase.landscape() && multiLine) {
            width = W_LAND_MULTI; //more editing space for landscape users
        } else {
            width = WIDTH;
        }

        PixelSpacebase.instance.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ...
                textInput = new EditText(PixelSpacebase.instance);
                ...
                PixelSpacebase.instance.addContentView(textInput, layout);
            }
        });
    }
        ...
        if (textInput != null) {
            PixelSpacebase.instance.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    InputMethodManager imm = (InputMethodManager) PixelSpacebase
