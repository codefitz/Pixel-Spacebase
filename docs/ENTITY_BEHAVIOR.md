...
| Blasters | Rechargeable ranged weapons with charge counts. Each blaster has a distinct projectile/effect and can be used by DM-3000 launcher mechanics. A Shapeshifter cannot fire one normally; throwing it consumes the blaster and beams all hostile mobs in the impact room down to 0 HP. |
...
| Armor | Defensive equipment with strength requirements, upgrades, malfunctions, and enhancements. Space suits and hunter suits protect biological heroes on exposed bridge walks outside the station and from harmful gas. The Hunter Suit adds one ranged attack-skill point, shows red silhouettes for enemies and NPCs through fog while floor lights are off without revealing terrain or changing targeting sight, and toggles unlimited jetpack flight while worn. Removing it or powering down over a chasm triggers a fall. A uniform makes unprovoked machine mobs ignore the wearer; each machine remembers if attacked across saves. With no armor, the hero moves 50% faster and has 25% greater dodge, without faster attacks. The Hoverpod absorbs landed hits and avoids non-explosive traps; its visible integrity starts at 15 hits, and it is destroyed in an explosion when depleted. Explosions bypass its protection. The Repair Blaster restores its integrity, and combining two Hoverpods at Leonard's workstation restores and increases maximum integrity by one. |
